...
public interface ScoreRepository extends CrudRepository<ScoreCard, Long> {
     ...
     * @return the total score for the user, empty if the user doesnt exist.
     */
     ...
     */
    @Query("SELECT NEW com.microservices.gamification.game.domain.LeaderBoardRow(s.userId, SUM(s.score))" + "" +
            "FROM ScoreCard s " +
